...
class JobExecutor:
    ...
    async def _download_files(self, job: Dict[str, Any], dest: Path) -> None:
        ...
        Uses the agent's JWT token for authentication.
        """
        headers = {}
        if self.agent_token:
            logger.info(
                "Download auth token present for job %s (length=%d, suffix=%s)",
                job.get("job_id", "unknown"),
                len(self.agent_token),
                self.agent_token[-4:],
            )
            headers["Authorization"] = f"Bearer {self.agent_token}"
        else:
            logger.warning(
                "No agent token configured for job %s downloads; request will be anonymous",
                job.get("job_id", "unknown"),
            )
